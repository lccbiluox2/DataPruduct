...
<ul>
<form action="/newsController/updateNewsById" method="post" "  enctype="multipart/form-data">
<li>新闻ID：
	<input type="text" name="id"  value="${item.id?if_exists}"/>
</li>
<li>新闻标题：
	<textarea  name="title"  cols="130" rows="4" > ${item.title?if_exists} </textarea> 
</li>
<li>新闻摘要：
	<textarea  name="zaiyao"  cols="130" rows="4" > ${item.zaiyao?if_exists} </textarea> 
</li>
<li>新闻时间：<input type="date" name="time" value="${item.time?if_exists}" /> </li>
<li>新闻内容：
	<textarea 	name="content" id="textarea" cols="140" rows="30">
		${item.content?if_exists}
	</textarea>
</li>
<li>新闻标签：
	<input type="text" name="tag"  style="width:800px;" placeholder="请输入以空格分割的标签" value="${item.tag?if_exists}"/>
</li>
